...
def strict_f1(actual, predicted,actual_evidence,predicted_evidence, label):
    ...
    for i in range(len(actual)):
      if actual[i]!="NOT ENOUGH INFO":
        if (actual[i]==label) & ((predicted[i]==label) & (bool(set(predicted_evidence[i]) & set(actual_evidence[i]))==True)):
           tp=tp+1
        elif (actual[i]!=label) & (predicted[i]==label):
           fp=fp+1
        elif (actual[i]==label) & ((predicted[i]==label) &  (bool(set(predicted_evidence[i]) & set(actual_evidence[i]))==False)):
           fp=fp+1
...
def f1_macro_strict(actual, predicted,actual_evidence,predicted_evidence):
    ...
    return np.mean([strict_f1(actual, predicted, actual_evidence,predicted_evidence, label) 
        for label in np.unique(actual)])
...
def main(pred_file, actual_file): 
 ...
 with open(args.actual_file, "r") as fr:
    risData = json.load(fr)
 
 #for line in jsonlines.open(args.actual_file):
 for line in risData:
 ...
 strict_macro_F1=f1_macro_strict(actual_labels,pred_labels,actual_evidence,pred_evidence)
